...
class seatClass:
    ...
    # count
    def __init__(self, xPos, yPos, width, height, seatShape, seatNum, seatInfo, seatCount, seatStepCount):
        self.xPos = xPos
        self.yPos = yPos
        self.width = width
        self.height = height
        ...
        self.seatInfo = seatInfo
        self.seatCount = seatCount
        self.seatStepCount = seatStepCount
        #self.count = count
                        ...
                        for j in range(len(ds[1])): #480
                            if (i, j) in posDict:
                                ...
                                if posDict[(i,j)] == "table": 
                                    ...
                                    request_shape([i+15 + plus_x, j-15 + plus_y, 90, 90, "table", cnt])
                                    # seatClass(xpos, ypos, width, height, seatShape, seatNum, seatInfo)
                                    seatList.append(seatClass(i-30, j-30, 60, 60, "table" ,cnt, "empty", 0))
                                    seatList.append([i+15 + plus_x, j-15 + plus_y, 90, 90, "table", cnt, None, 0])
                                    
                                elif posDict[(i,j)] == "longtable" : 
                                    request_shape([i-30 + plus_x, j-60 + plus_y, 90, 150, "longtable", cnt])
                                    seatList.append(seatClass(i-30, j-30, 60, 120, "longtable" ,cnt, state, seatCount))
                                    seatList.append([i+15 + plus_x, j+15 + plus_y, 90, 150, "longtable",cnt, None, 0])
                                    # seatList[-1].print()
